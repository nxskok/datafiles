...
scores.y=gather(scores,assessment,score,-test.2)
scores.y

###################################################
### code chunk number 3: bRscript.Rnw:188-190
###################################################
separate(scores2,assessment,into=c("assess.type","assess.num"),sep="\\.")

# other variations on sep:
scores.x=scores2

scores.x$a2=c(rep("a1",4),rep("t1",4),rep("t2",4))
scores.x
separate(scores.x,a2,into=c("assess.type","assess.num"),
         sep=1)
scores.x$a3=c(rep("assgtnumber1",4),rep("testnumber1",4),rep("testnumber2",4))
scores.x
separate(scores.x,a3,into=c("assess.type","assess.num"),
         sep="number")
...
scores2
outof=c(rep(10,4),rep(50,8))
outof
mutate(scores2,out.of=outof,
               percent=score/out.of*100)

scores2 %>% separate(assessment,into=c(
  "assess.type","assess.number"),sep="\\.") %>%
  mutate(out.of=ifelse(assess.type=="assgt",10,50),
         percent=score/out.of*100
        )
...
scores2 %>% summarise(mean.score=mean(score))
